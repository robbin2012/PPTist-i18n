...
- `hasItemNote`：是否有列表项备注（textType: 'itemNote'）
...
- `item`：列表项内容
- `itemTitle`：列表项标题
- `itemNote`：列表项备注（可选，附加在对应列表项上的补充说明/提示）
- `itemNumber`：列表项编号或年份
- `notes`：**模板自定义生成规则**（新增）- 用于指导AI生成内容的特殊说明（不会写回元素）
...
- **列表项**：
  - 仅正文：`"项目内容"`（字符串）
  - 标题 + 描述：`{ "title": "...", "text": "..." }`
  - 标题 + 描述 + 备注：`{ "title": "...", "text": "...", "note": "..." }`（可选备注，对应模板中的 `itemNote` 元素）
- **适用场景**：要点罗列、特性介绍、步骤说明
...
1. **正确标记元素的textType**：
   ...
   - 可选：`itemNote`（列表项备注，附加在对应列表项上的短句说明）
   - 可选：`notes`（模板级生成规则提示，不会直接写回到元素）
