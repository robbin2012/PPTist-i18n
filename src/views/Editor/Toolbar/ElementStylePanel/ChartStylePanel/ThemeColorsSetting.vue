<template>
  <div class="theme-colors-setting">
    <div class="title">{{ t('toolbar.chart.themeColorsSetting') }}</div>

    <div class="list">
      <div class="row" v-for="(item, index) in themeColors" :key="index">
        <div class="label" style="width: 40%;">{{ t('toolbar.chart.themeColorN', { n: index + 1 }) }}：</div>
        <Popover trigger="click" style="width: 60%;">
          ...
          <div class="color-btn-wrap" style="width: 100%;">
            ...
            <div class="delete-color-btn" v-tooltip="t('common.delete')" @click.stop="deleteThemeColor(index)" v-if="index !== 0"><IconCloseSmall /></div>
          </div>
        </Popover>
      </div>
      <Button
        ...
        @click="addThemeColor()"
      >
        <IconPlus /> {{ t('toolbar.chart.addThemeColor') }}
      </Button>
    </div>
