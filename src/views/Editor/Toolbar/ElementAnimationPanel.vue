  ...
  <div class="element-animation-panel">
    <div class="element-animation" v-if="handleElement">
      ...
      >
        ...
        <Button class="element-animation-btn" @click="handleAnimationId = ''">
          <IconEffects /> {{ t('animation.add') }}
        </Button>
      </Popover>
    </div>

    <div class="tip" v-else><IconClick style="margin-right: 5px;" /> {{ t('animation.selectToAdd') }}</div>
      ...
      <template #item="{ element }">
        <div class="sequence-item" :class="[element.type, { 'active': handleElement?.id === element.elId }]">
          <div class="sequence-content">
            ...
            <div class="handler">
              <IconPlayOne class="handler-btn" v-tooltip="t('animation.preview')" @click="runAnimation(element.elId, element.effect, element.duration)" />
              <IconCloseSmall class="handler-btn" v-tooltip="t('thumbnails.context.delete')" @click="deleteAnimation(element.id)" />
            </div>
          </div>
          ...
          <div class="configs" v-if="handleElementAnimation[0]?.elId === element.elId">
            ...
            <div class="config-item">
              ...
              <Select
                ...
                style="width: 65%;"
                :options="[
                  { label: t('animation.triggers.click'), value: 'click' },
                  { label: t('animation.triggers.meantime'), value: 'meantime' },
                  { label: t('animation.triggers.auto'), value: 'auto' },
                ]"
              />
            </div>
            <div class="config-item">
              <Button style="width: 100%;" @click="openAnimationPool(element.id)"><IconSwitch /> {{ t('animation.change') }}</Button>
            </div>
          </div>
        </div>
      </template>
    ...
    <template v-if="animationSequence.length >= 2">
      ...
      <Button @click="runAllAnimation()">
        {{ animateIn ? t('animation.stopPreview') : t('animation.previewAll') }}
      </Button>
    </template>
  </div>
</template>
...
const tabs: TabItem[] = [
  ...
  { key: 'attention', label: t('animation.tabs.attention'), color: '#e8b76a' },
]
...
const animationSequence = computed(() => {
  ...
  for (let i = 0; i < formatedAnimations.value.length; i++) {
    ...
    for (let j = 0; j < item.animations.length; j++) {
      ...
      const elType = t(`elements.type.${el.type}`)
